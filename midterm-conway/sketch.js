  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
     ...
     // write pixels into destination image, scaled to 0 or 255:
      img[1-whichimage].set(i, j, color(Math.floor(random(0,255)),Math.floor(random(0,255)),Math.floor(random(0,255)))), (color(Math.floor(random(0,100)))); 
    }
  }
...
function fillatmouse()
{
  ...
  img[whichimage].set(thex, they, color(100));
